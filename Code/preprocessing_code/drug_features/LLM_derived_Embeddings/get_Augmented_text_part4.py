...
CSV_PATH            = Path("./RawData/compounds_df_wTarMoA_part4.csv")          # full dataset
SYSTEM_PROMPT_PATH  = Path("./RawData/Jul9_WholeData/data/prompt.txt")
...
compounds_info_part4 = pd.read_csv(
    CSV_PATH,
    index_col=0,
...
try:
    ...
    iterator = tqdm(compounds_info_part4.itertuples(index=False), total=len(compounds_info_part4))
except ImportError:
    iterator = compounds_info_part4.itertuples(index=False)
...
compounds_info_part4["augmented_yaml"] = yaml_list

compounds_info_part4.to_csv("./RawData/compounds_with_yaml_part4.csv", index=False)
