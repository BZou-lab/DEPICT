...
parser.add_argument("--num_epochs", type=int, default=50,
                        help="Total number of epochs (must be > 0).")
parser.add_argument("--start_epoch", type=int, default=50,
                        help="Epoch to start from (>= 0 and < num_epochs).")
...
## continue training
resume = True          # set to False for fresh runs
start_epoch = args.start_epoch

if resume and Path(best_model_path).exists():
...
rest_num_epoch = num_epochs + start_epoch - 1

## training loop
for epoch in range(start_epoch, rest_num_epoch + 1):

    print(f"\n Epoch {epoch}/{rest_num_epoch} starting...")
...
loss_csv_path = f"./Model/TrainingLoss/transformer_d32h8l4_losses_{split_type}_dp1_MSECor_lr1_CosSche_3XAttn_sepGene2EncPred_newAttn_FiLM_Enc3DimReducLa128_CellAware_frontEndMLPsimp_whole_DoseTimeAsScalarXattns_LLMMFP_rest150epoch.csv"
