'''
This is the training code that fit our data
Modified by Meisheng, Aug10 2025.
1. changed the argparser, split function and the output dir
2. mute the seed setting
'''
...
def parse_args():
    ...
    parser.add_argument("--eval_metric_valid", type=bool, default=False)
...
def train_TranSiGen(args):
    ...
    eval_metric_valid = args.eval_metric_valid
    ...
    # Out dir
    local_out = '../results/trained_models_{}_epoch_{}/feature_{}_init_{}/'.format(split_type, n_epochs, feat_type, init_mode)
    ...
    if eval_metric_valid:
        print('===============Evaluate model performance==============')
        # setup_seed(random_seed)
        _, _, valid_metrics_dict_ls = model.test_model(loader=valid_loader, metrics_func=['pearson', 'rmse', 'precision100'])


        for name, rec_dict_value in zip(['valid'], [valid_metrics_dict_ls]):
            ...
            df_rec.to_csv(save_dir + '/{}_restruction_result_all_samples_validation.csv'.format(name), index=False)
